...
#ifdef VENDOR_EDIT
#Xuefeng.Peng@PSW.AD.Storage.1135933, 2017/11/08, Add for file mtime is not system local time
function touchfile() {
    needtouchfile=`getprop persist.sys.touchfile`
    if [ x"${needtouchfile}" = x"" ]; then
        #setprop persist.sys.touchfile "true"

        #echo "touch test start" >> /data/media/0/touchfile.txt

        touch /data/media/0/DCIM/Camera
        touch /data/media/0/DCIM/Screenshots
        touch /data/media/0/DCIM
        touch /data/media/0/Music
        touch /data/oppo_log

        #DATE=`date +%F-%H-%M-%S`
        #echo "${DATE}" >> /data/media/0/touchfile.txt
        #echo "touch test end" >> /data/media/0/touchfile.txt
    fi
}
#endif /*VENDOR_EDIT*/
